...
export default function AgentDashboard({
  ...
  const hasScoreData = myMonitorias.some((m: any) => m.score !== undefined && m.score !== null);
  const hasReversalData = (contestationsApproved + contestationsRejected) > 0;
          ...
          badge={
            isCustomizing ? (
              <span className={`inline-flex items-center justify-center gap-0.5 px-1.5 py-0.5 text-[10px] font-bold rounded-md self-center ${isCustomizing ? 'bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400' : revColorClass}`}>
                {isCustomizing ? '↑' : revSign} {isCustomizing ? '5.42%' : Math.abs(revDiff).toFixed(2)}%
              </span>
            ) : hasReversalData ? (
