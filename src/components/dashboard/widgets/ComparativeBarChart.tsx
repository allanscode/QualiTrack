    ...
    <Card padding="lg" className="h-full flex flex-col">
        ...
        (title || subtitle) && (
          <div className="flex items-center gap-3 mb-4 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              {title && <h3 className="text-sm font-black text-brand-primary uppercase tracking-widest whitespace-normal leading-snug" title="">{title}</h3>}
            </div>
          </div>
        )
      )}
      <div className="flex-1 min-h-[150px]" style={{ minWidth: 0, minHeight: 150 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }} barGap={4} barSize={20}>
                    ...
                    <div className="space-y-1">
                      {payload.map((p: any, i: number) => (
                        <div key={i} className="flex items-center gap-2">
                          <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: p.fill || p.color }} />
